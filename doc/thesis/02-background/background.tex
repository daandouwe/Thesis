...
\begin{figure}[h]{\textwidth}
  ...
  \begin{tikzpicture}[scale=.8]
    \Tree [.x [.x A bird ] [.x hit [.x the car ] ] ]
  \end{tikzpicture}
...
\say{In sentence \ref{fig:tree-npi-licensed} the hierarchical structure dominating \textit{not} also immediately dominates the hierarchical structure containing \textit{anybody}. In sentence \ref{fig:tree-npi-unlicensed}, by contrast, \textit{not} sequentially precedes \textit{anybody}, but the triangle dominating \textit{not} fails to also dominate the structure containing \textit{anybody}' }
