...
\section{Neural networks}
  ...
  \subsection{Optimization}
    We use the neural networks to parametrize probability distributions, subtly summarized by the set of parameters $\theta$ in a probability mass function $p_{\theta}$. These parameters are optimized using gradients. Our prototypical objective is optimization of the maximum likelihood of a set observations
    \begin{align*}
      \objective(\theta) = \sum_{\x \in \dataset} \log \ptheta(x)
    \end{align*}
    ...
    \end{align*}
    where $\x^{(k)}$ are sampled uniformly from $\dataset$, making our optimization procedure stochastic. This works because for uniformly sampled $x^{(s)}$
    \begin{align*}
      \nabla_{\theta} \mathcal{L}(\theta)
        &\propto \expect [ \nabla_{\theta} \log \ptheta (\x^{(s)}) ].
    \end{align*}
    For this estimate we use minibatches. Under certain conditions on the learning rate $\gamma^{(t)}$ this method is guaranteed to converge to a local optimum \citep{robbins1951stochastic}.
