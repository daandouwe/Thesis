...
\section{Hypergraph}
  ...
  \begin{example}{\textbf{(CRF parser})
    \label{ex:crf}
    ...
    \end{align*}
    This set of edges is illusrated in figure \ref{fig:edges} and figure \ref{fig:hypergraph} shows a fragment of this hypergraph for an example sentence. Shown are the two hyperpaths corresponding to the two (overlapping) derivations
    \begin{align}
...
\section{Semiring parsing}
  ...
  \begin{definition}{\textbf{(Hypergraph weights)} Let $G_{\omega} = (V, E, \omega)}$ be a weighted hypergraph, with $\omega$ defined over a semiring $\mathcal{K}$. We define the weight of the derrivation $D \subseteq E$ as the product of the weights of the edges:
  \begin{align}
...
\begin{example}{\textbf{(Logartihmic domain)}}
  ...
  In the CRF, the function $\omega$ is given by the composed function $\log \circle \Psi$.
\end{example}

\begin{example}{\textbf{(Viterbi weight)}}
...
\begin{example}{\textbf{(Viterbi derivation)}}
...
\begin{example}{\textbf{(Recognition)}}
