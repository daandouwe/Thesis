...
\section{Optimization}
  ...
  \begin{align}
    \label{eq:surrogate}
    \objective_{ \textsc{surr} }(\theta, \lambda) &= \frac{1}{K}\sum_{k=1}^K \log \qlambda(\x | \y^{(k)}) \blockgrad( L(\x, \y^{(k)}) ).
  \end{align}
  ...
  \begin{align*}
    \nabla_{\lambda} \objective_{ \textsc{surr} }(\theta, \lambda) &= \frac{1}{K}\sum_{k=1}^K L(\x, \y^{(k)}) \nabla_{\lambda} \log\qlambda( \x | \y^{(k)} ),
  \end{align*}
