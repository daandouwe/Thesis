...
\section{Model}
  ...
  \subsection{Model}
    Fundamentally, the model is a probability distribution over transition action sequences $a = \langle a_1, \dots, a_T \rangle$ that generate trees $\y$. Conditionally---given a sequence of words $\x$---in the discriminative model, and jointly---predicting the sequence $\x$---in the generative model. Put simply, the model is thus defined as
    \begin{equation}
...
\section{Parametrization}
  ...
  \subsection{Composition function}
    ...
    The attention-based composition is defined as follows. Let $[ \vech_1, \dots, \vech_m ]$ be the vector representations computed by a dedidacted bidirectional LSTM for the $m$ words making up the constituent that is to be compressed, and let $\mathbf{n}$ be the learned embedding for the nonterminal category of that constituent. The attention weight $a_i$ for the $i$-th position is computed as the bilinear product
    \begin{align*}
      a_i &= \frac{a_i}{\sum_{i=1}^m \tilde{a}_i}  \\
      \tilde{a}_i &= \exp \{ \vech_i^{\top} \mathbf{V} [\vecu_t ; \mathbf{n}] \}
    \end{align*}
    between the vector representation $\vech_i$ and the concatenation of current parser state representation $\vecu_t$ and nonterminal representation $\mathbf{n}$. The matrix $\mathbf{V}$ is part of the trainable parameters. The vector
    \begin{align*}
      \mathbf{m} = \sum_{i=1}^m a_i \mathbf{h}_i
    \end{align*}
    then represents the words in the constituent, and the final representation of the constituent is the gated sum
    \begin{align*}
      \mathbf{c} = \mathbf{g} \circ \mathbf{n} + (1 - \mathbf{g}) \circ \mathbf{m}.
    \end{align*}
    of this vector and the vector $n$.\footnote{The operation $\circ$ denotes the Hadamard product, or elementwise multiplication: $(A \circ B)_{ij} = (A)_{ij}(B)_{ij}$.} The vector of gates $\mathbf{g}$ is computed by an affine transformation on $[ \mathbf{n}; \mathbf{m} ]$ followed by an elementwise application of the logistic function.\footnote{Our definition of $\mathbf{c}$ is slightly different from that of \citet{kuncoro2017syntax}: where they use another vector representation $\mathbf{t}$ different from $\mathbf{n}$ in the above definition; we use just $n$ to compute both the attention weights and the final vector.}.
...
\section{Experiments}
  This section reports the results of the experiments performed with our own implementation of the RNNG. We train the discriminative and generative model on the Penn Treebank with the same hyperparameters as \citep{dyer2016rnng} and compare our results to conclude the correctness of our implementation. We then investigate how the approximate marginalization depends on two parameters: the number of samples, and the temperature used to scale the distribution of the proposal model. For the general training setup, including details about data preprocessing, we refer the reader to appendix \ref{A2-implementation}.

  \subsection{Models}
    We use the same hyperparameters as \citep{dyer2016rnng}. For both models the embeddings are of size 100, and the LSTMs have 2 layers. The LSTMs have dimension 128 in the discriminative model and 256 in the generative model, and the dimension of the feedforward networks that compute the action distributions are the same as the LSTMs. We apply dropout to all layers including the recurrent connections\footnote{Also called \textit{variational dropout} \citep{gal2016theoretically}, which is implemented by default in Dynet \citep{neubig2017dynet}.} and use a dropout ratio of 0.2 in the discriminative model, and 0.3 in the generative model. Given the above dimensions, the total number of trainable parameters is approximately 800,000 (798,078) for the discriminative model and 9,500,000 (9,610,736) for the generative model.

    Our discriminative model does not make use of predicted tags, which contrasts with \citet{dyer2016rnng}. This decision was made with the application of the RNNG as language model in mind, in which case tags are not available, but this choice will affect the parsing accuracy of the discriminative model. For a more elaborate justification of this choice we refer the reader to section \label{sec:impl-embedding} of the implementation appendix.

  \subsection{Results}
    We train 10 separate models from different random seeds.\footnote{The random seeds control both the random initialization of the model parameters, as well as the order in which the training data is presented.} We report mean and standard deviation to give a sense of the variability of the accuracy, as well as the value of the model with the best development performance. Model selection is based on development F1 for the discriminative model, and based on development perplexity for the generative model. The parsing F1 of the discriminate model can be seen in table \ref{tab:disc-fscores}. The result of the generative model can be seen in tables \ref{tab:gen-fscores} (F1) and \ref{tab:gen-perplexities} (perplexity).
    ...
    The accuracy of our discriminative model is below that of \citet{dyer2016rnng}, a gap that can reasonably be attributed to the absence of tag information in our model. This gap in F1 is carried through to the generative model, resulting in a lower F1 than reported by \citet{dyer2016rnng} when using our own samples. The reranking by the generative models still finds better trees than the greedy decoding of the discriminative model, as evidenced by the higher F1 score, and when we evaluate with the proposal samples used in the original work we find that the F1 agrees perfectly. The difference in perplexity is much less pronounced, and our samples 

    The generative models are evaluated using proposal samples. The quality of these samples affects the model's parsing F1 and the accuracy of the approximate marginalization.
