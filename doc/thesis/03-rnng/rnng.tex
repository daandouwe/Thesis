...
\section{Model}
  ...
  \subsection{Model}
    Fundamentally, the model is a probability distribution over transition action sequences $a = \langle a_1, \dots, a_T \rangle$ that generate trees $\y$.  Conditionally---given a sequence of words $\x$---in the discriminative model, and jointly---predicting the sequence $\x$---in the generative model. The model then defines a distribution over trees $\yieldx$ through the bijective transformation that maps transition sequences $a$ to trees $y$ and vice versa.\footnote{ As a result of the transition system, the trees in the set $\yieldx$ are totally unrestrained: the trees can have any arity, and can contain arbitrarily long unary chains. This contrasts with grammar based parsing models that do put such restrictions on the trees in their support.} Put simply, the model is thus defined as
    \begin{equation}
...
\section{Experiments}
  ...
  \subsection{Samples}
    ...
    \begin{figure}[h]
      \center
    	\includegraphics[width=0.5\textwidth]{sample-experiment/entropy/disc_temp.pdf}
