...
\section{Experiments}
  ...
  \subsection{Results}
    ...
    The parsing F1 of the discriminate model can be seen in table \ref{tab:disc-fscores}, and the results of the generative model can be seen in tables \ref{tab:gen-fscores} (F1) and \ref{tab:gen-perplexities} (perplexity). The value between brackets is the value based on model selection.

    The accuracy of our discriminative model is below that of \citet{dyer2016rnng}, a gap that can reasonably be attributed to the absence of tag information in our model. This gap in F1 is carried through in a lesser degree to the generative model, resulting in a lower F1 than reported by \citet{dyer2016rnng} when using our own samples. The reranking by the generative models still finds better trees than the greedy decoding of the discriminative model, as evidenced by the higher F1 score, and when we evaluate with the proposal samples used in the original work we find that the F1 agrees perfectly with that of \citet{dyer2016rnng} and is on par with the results reported in \citet{kuncoro2017syntax}.\footnote{The proposal samples used by \citet{dyer2016rnng} are availlable at \url{https://github.com/clab/rnng}.}. The difference in perplexity performance is less pronounced, and with our own samples we manage to get close to the perplexity published in \citet{dyer2016rnng}. The perplexity does lag behind the results reported in \citet{kuncoro2017syntax}, which are the results obtained with the attention-based composition function.

  \subsection{Samples}
    How do the number of samples and the temperature $\alpha$ influence the approximate inference? To gain insight into this dependence we performed the following experiment. We evaluate the generative RNNG as in the above setting, while varying the number of samples, and the temperature $\alpha$: we let the number of samples be in $\{ 10, 25, 50, 75, 100 \}$ and let $\alpha$ be in $\{ 0.8, 1 \}$.\footnote{When $\alpha=1$, the distribution is left unchanched.}. To quantify the variability of the proposal model in this we repeat the experiment 10 times. To quantify how the spread of the  can use these samples

    Figure \ref{fig:samples-fscores} shows the results of this experiment for the parsing F1, and \ref{fig:samples-perplexities} shows the results for the perplexity.

    \begin{figure}[h]
      \center
    	\includegraphics[width=0.9\textwidth]{sample-experiment/fscore/disc_temp.pdf}
    \caption{F1 for varying number of samples, with and without annealed distributions, for 10 independent repetitions. Horizontal lines show values of individual outcomes.}
    ...
    \caption{Perplexity for varying number of samples, with and without annealed distributions, for 10 independent repetitions. Horizontal lines show values of individual outcomes.}
