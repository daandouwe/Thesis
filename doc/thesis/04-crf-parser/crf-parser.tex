...
\subsection{Solutions}
  Equiped with the above two recursions and a range of semirings we can provide the solutions promised at the outset of this section.
  ...
  \paragraph{Parse}
    To find the viterbi tree $\hat{ \y } = \arg \max_{\y } p(\y  \mid \x)$ and its probability $p(\hat{\y} \mid \x)$ we use the Viterbi semirings ($\cf$ examples \ref{ex:vit-weight} and \ref{ex:vit-derivation} in appendix \ref{A3-crf}). We take equation \label{eq:inside-simplified} and substitute the Viterbi semiring operations to derive that the value of the best subtree spanning words $i$ to $j$ is given by
    \begin{align}
      \label{eq:viterbi-score}
      \nu(i,j)
        &= \max_{A} [ \log \psi(A, i, j) ] + \max_{k} [\nu(i,k) + \nu(k,j)].
    \end{align}
    The value $\log p(\hat{\y} \mid \x)$ is then given by $\mu(0, n)$. The best label and splitpoint $\hat{A}$ and $\hat{k}$ for the span $(i, j)$ are obtained by using the argmax:
    \begin{align}
  ...
  \paragraph{Sample}
    Samples can be obtained by recursively sampling edges, starting at the root node $(S^{\dagger}, 0, n)$. The probability of an edges is proportional to the weight under that edge: this is precicely the inside value $\alpha$ computed in the real-semiring. We thus sample an edge $e = \langle \{ (B, i, k), (C, k, j) \},  (A, i, j) \rangle$ with probability
    \begin{align}
      \label{eq:sample}
      P(e)
        &\propto \omega(e) \otimes \displaystyle\bigotimes_{u \in T(e)} \alpha(u)  \nonumber  \\
        &= \psi(A, i, j) \, \alpha(B, i, k) \, \alpha(C, k, j)  \nonumber  \\
  ...
  \paragraph{Entropy}
    To compute the entropy $H(P(Y \mid X = x))$ over parses for the sentence $\x$ we first introduce the notion of the \textit{maginal probablity} of a node, the first and only quantity on we will encounter that also requires the values $\beta$ computed by the outside recursion. The posterior marginal of a node $v = (A, i, j)$ in a hypergraph $G$ is the probability that it occurs in a tree $\y$ for the sentence $\x$ as governed by the distribution $p$. We introduce therandom variable $V$, with the hypergraph nodes $\mathcal{V}$ as sample space, and define
    \begin{align}
      P(V = v \mid X = \x)
        &\defeq \expect_Y[ \indicator_{ \{ v \in Y \} } ]  \nonumber \\
        &= \sum_{ \y \in \yieldx } p(\y \mid \x) \indicator_{ \{ v \in y \} }.
    \end{align}
    The marginals can be computed from the inside and outside values computed in the real semiring as \citep{baker1979trainable,goodman1999semiring}
    \begin{align}
      P(V = v \mid X = \x) = \frac{\alpha(A, i, j) \, \beta(A, i, j)}{Z(\x)},
    \end{align}
    which can be seen by noting that the product of $\alpha(A, i, j)$ and $\beta(A, i, j)$ is the sum over all trees that contain the node $v$, and $Z(\x)$ the sum over all trees in general. Using the marginals, the entropy of the distribution over parses can be written as
    \begin{align}
      H(P(Y \mid X = x))
        &= - \sum_{ \y \in \yieldx } p(\y \mid \x) \log p(\y \mid \x)  \nonumber \\
...
%     &= \tilde{s}(i, j, A) \cdot \sum_{k=i+1}^{j-1} \sigma(i,k) \cdot \sigma(k,j)
% \end{align*}
%
...
%   \tilde{s}(i, j, A)
%     &= \exp( s(i, j, A) ) \\
%     \sigma(i,j) &= \sum_{A \in N} \alpha(A,i,j)
% \end{align*}
%
% From equation \ref{eq:inside} we can deduce that we in fact do even need to store the values $\alpha(i, j, A)$ but that it suffices to only store the marginalized values $\sigma(i, j)$. In this case, the recursion simplifies even further:
% \begin{align*}
%     \sigma(i, j) &= \sum_{A \in N} \alpha(A,i,j) \\
%         &=  \sum_{A \in N} \tilde{s}(i, j, A) \cdot \sum_{k=i+1}^{j-1} \sigma(i,k) \cdot \sigma(k,j) \\
%         &= \Bigg[ \sum_{A \in N} \tilde{s}(i, j, A) \Bigg] \cdot \Bigg[\sum_{k=i+1}^{j-1} \sigma(i,k) \cdot  \sigma(k,j) \Bigg]
% \end{align*}
% where we put explicit brackets to emphasize that independence of the subproblems of labeling and splitting.
%
% \subsection{Inside recursion}
...
%     \alpha(A,i,j) &= \sum_{B \in N} \sum_{C \in N} \sum_{k=i+1}^{j-1} \tilde{s}(i, j, A) \cdot \alpha(B,i,k) \cdot \alpha(C,k,j) \\
%         &= \tilde{s}(i, j, A) \cdot \sum_{k=i+1}^{j-1} \sum_{B \in N} \alpha(B,i,k) \cdot \sum_{C \in N} \alpha(C,k,j) \\
%         &= \tilde{s}(i, j, A) \cdot \sum_{k=i+1}^{j-1} \sigma(i,k) \cdot \sigma(k,j) \label{eq:final-inside}
% \end{align*}
% \end{subequations}
% where we've introduced a number of notational abbreviations
% \begin{align*}
...
%
% From equation \ref{eq:final-inside} we can deduce that we in fact do even need to store the values $\alpha(i, j, A)$ but that it suffices to only store the marginalized values $\sigma(i, j)$. In this case, the recursion simplifies even further:
% \begin{subequations}
% \begin{align*}
%     \sigma(i, j) &= \sum_{A \in N} \alpha(A,i,j) \\
%         &=  \sum_{A \in N} \tilde{s}(i, j, A) \cdot \sum_{k=i+1}^{j-1} \sigma(i,k) \cdot \sigma(k,j) \\
%         &= \Bigg[ \sum_{A \in N} \tilde{s}(i, j, A) \cdot \Bigg] \Bigg[\sum_{k=i+1}^{j-1} \sigma(i,k) \cdot  \sigma(k,j) \Bigg]
% \end{align*}
% \end{subequations}
...
%           &=  \sum_{k=1}^{i-1}  \sigma'(k, j) \cdot \sigma(k, i-1) + \sum_{k=j+1}^{n} \sigma'(i, k) \cdot  \sigma(j+1, k) \\
%   \end{align*}
%   where
%   \begin{align*}
%       \sigma(i, j) &= \sum_{A \in N} \alpha(A, i, j) \\
%       \sigma'(i, j) &= \sum_{A \in N} \psi(A, i, j) \beta(A, i, j)
%   \end{align*}
