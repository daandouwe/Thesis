...
class Parseval(object):
    ...
    def load_trees(self, path):
        with open(path) as f:
            # convert to span trees. FIXME: this also collapses unaries...
    ...
    def score(self, gold_tree, pred_tree):
        gold_spans = gold_tree.spans()
        pred_spans = pred_tree.spans()
        correct = set(pred_spans) & set(gold_spans)  # FIXME: uncollapse unaries before intersection
